...
def create_roller_model(simulation_file_name, geometry_file_name, material, p0, rolling_angle):
    ...
    rotation_matrix = np.matmul(rotation_matrix_y, rotation_matrix_x)
    rot_vector = np.array([rotation_matrix[2, 1] - rotation_matrix[1, 2],
                           rotation_matrix[0, 2] - rotation_matrix[2, 0],
                           rotation_matrix[1, 0] - rotation_matrix[0, 1]])
    rot_vector /= np.linalg.norm(rot_vector)
    angle = np.arccos((rotation_matrix[0, 0] + rotation_matrix[1, 1] + rotation_matrix[2, 2] - 1)/2)*180/np.pi
    file_lines.append('\t\t0., 0., 0.,  ' + str(rot_vector[0]) + ', ' + str(rot_vector[1]) + ', '
                      + str(rot_vector[2]) + ', ' + str(angle))
    ...
    file_lines.append('\t\t' + str(np.cos(q)) + ', 0., ' + str(-np.sin(q)) + ', ' +
                      str(np.sin(q)) + ',  0., ' + str(np.cos(q)))
    ...
    file_lines.append('*Amplitude, name=rolling_amp')
    file_lines.append('\t0.0, 0.0')
    file_lines.append('\t1.0, 0.0')
    file_lines.append('\t2.0, 1.0')
    file_lines.append('*Amplitude, name=load_amp')
    file_lines.append('\t0.0, 0.0')
    ...
    file_lines.append('\troller_ref_node, 1, 1')
    file_lines.append('\troller_ref_node, 3, 5')
    ...
    file_lines.append('\t*Static')
    file_lines.append('\t\t1e-2, 2., 1e-12, 1.')
...
if __name__ == '__main__':
    ...
    create_roller_model(simulation_directory + 'roller_model_no_trans.inp', model_file, SS2506_no_trans, 2000, 10)
