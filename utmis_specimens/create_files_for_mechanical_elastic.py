...
specimen_loads = {'smooth': {-1.: [1., 100., 1000.], 0.: []},
                  'notched': {-1.: [1., 100., 1000.], 0.: []}}
    ...
    def write_inp_file(force):
        ...
        for step in range(no_steps):
            for direction in ['max_load', 'min_load']:
                ...
                file_lines.append('\t*Output, field, frequency=10')
...
def write_run_file(job_names, heat_treatment_file, directory, cpus=12):
    ...
    for job_name in job_names:
        file_lines.append('${abq} j=' + job_name + ' cpus=' + str(cpus))
...
if __name__ == '__main__':
    heat_treatment_data_file = os.path.expanduser('~/utmis_specimens/' + specimen + '/Toolbox_Cooling_utmis_' + specimen
                                                  + '.htd')
    simulation_directory = os.path.expanduser('~/utmis_specimens/' + specimen + '/mechanical_analysis_elastic/')
